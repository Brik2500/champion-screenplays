...

{
  ...
  "primaryWeaknesses": [
    "<2-4 specific weaknesses: name the element, the root cause, and the audience consequence. Apply both tests below before listing anything as a weakness:>",
    "<SCENE LENGTH TEST: Before listing any scene as a weakness for pacing or length, ask — does the scene deliver (1) new information that changes our understanding, (2) a power/situation reversal, or (3) emotional escalation? If it delivers any of the three, it is NOT a pacing weakness. Courtroom scenes, procedural sequences, and sustained dialogue exchanges almost always deliver all three. Only list a scene as a pacing weakness if it fails all three tests.>",
    "<GENRE-BLEND TONAL SHIFT TEST: Before listing any tonal shift, register change, or genre-mode transition as a weakness, ask — is this script submitted as a genre blend (e.g. Comedy/Thriller, Horror/Comedy, Action/Drama)? If yes, movement between those genre registers is intentional design, not a flaw. NEVER list 'tonal shifts between comedy and thriller,' 'rapid genre shifts,' 'comedic tropes diluting thriller elements,' or 'tonal imbalance' as weaknesses in a genre-blend script. These phrases describe the genre working as intended. The only valid tonal weakness in a genre-blend script is a transition that is technically broken — mid-scene with no emotional trigger, or a shift that directly contradicts established character logic.>"
  ],
  "coreThemes": [
  ...
  "compsAnchor": "<2-3 specific comparable titles that match this script's genre, tone, mode, AND emotional register — not just surface subject matter. A valid comp requires at least 3 of: same genre, same tonal register, same structural mode, same audience, same platform tier. 'Single parent + child' alone is not a comp basis. 'Gifted child' alone is not a comp basis. 'Absurdist situations' alone is not a comp basis. The emotional texture, dramatic weight, and narrative approach must align. For dark comedy-thrillers: comps must be dark and thriller-adjacent — The Good Place, Parks and Recreation, Brooklyn Nine-Nine, and similar warm network comedies are categorically invalid comps for dark/edgy/thriller-register scripts regardless of any surface overlap. Valid dark comedy-thriller comps: Barry, Killing Eve, Russian Doll, Dead to Me, Search Party, The Flight Attendant, Fleabag. Name each comp and the specific 3+ overlap points that make it valid.>",
